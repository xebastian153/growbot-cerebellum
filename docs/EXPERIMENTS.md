...
100 ms every model is within a few points of its own twin floor, including doing nothing**:
...
over the linear model — +0.8 roll on the twin (96.0 vs 95.3) — is −0.6 on the real log. A
100 ms score on this walk measures how little the real body moves in 100 ms, not the model;
walk-3 has no walking segment, so this is a walk-1 statement. **At 500 ms persistence beats
