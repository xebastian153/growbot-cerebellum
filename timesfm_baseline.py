...
Needs the optional extra (`pip install -e ".[timesfm]"`); the model (200M params) is
downloaded on first run. The import happens inside main() so `--help` works without it.
"""
import argparse
import json
import time
...
def main():
    ...
    import timesfm  # optional extra; imported after argparse so --help never needs it
    ...
    (RESULTS / "timesfm_baseline.json").write_text(json.dumps(
        {"horizons": args.horizons, "n": len(starts), "ctx": args.ctx, "rows": res,
         "provenance": provenance(seeds={"starts": 0, "mlp": 0})}, indent=1))
